...
`workload` builds programs under `bb-tests/workloads`. Currently this includes CTest, MLIRTest, ModelTest, and tutorial.
...
- MLIRTest: compiler operator tests for verifying compiler implementation. Input is MLIR; checks that Linalg, Tile, Buckyball lowering layers correctly reach RISC-V workloads.
...
`workload clean` clears the workload output directory for one chip. It only deletes `bb-tests/output/<chip>`, not `bb-tests/workloads/build/<chip>`. CMake/Ninja build cache is preserved so the next `workload build` can reuse the existing build directory.
...
`workload build` compiles workloads. The command enters `bb-tests/workloads/build/<chip>`, runs `cmake -G Ninja ..` and `ninja`, and builds with CMake + Ninja. Usage:
...
`--chip` is required. Without `--model`, builds all workloads for the specified chip except end-to-end models, including tutorial, CTest, MLIRTest, embench, coremark, and other regular test programs. After build, `*-baremetal` and `*-linux` executables are synced to `bb-tests/output/<chip>/workloads`. For chip workloads under `*/chips/<chip>`, only the directory for `--chip` is synced.
...
The command scans all ELFs under `bb-tests/output/<chip>/workloads/src` whose names end with `-baremetal`, and writes corresponding `.hex` files in the same directory. When booting an OS, test cases are packaged into a full kernel flashed to FPGA; kernel `tohex` conversion is implemented in `bbdev kernel`.
